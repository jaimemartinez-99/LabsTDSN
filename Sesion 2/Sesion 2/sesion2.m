...
stem(n,filtro,'k','MarkerfaceColor','k'),
...
for s=1:1:4 % falta código
    r=randn(1,L);
    for b=0:L-1
        sumx=sumx + (A*cos((2*pi/20)*b))^2;
        sumr= sumr + r(b+1).^2;
    end
  
    g= sqrt(sumx/(10^(SNR(s)/10)*sumr));
    y=x+g*r;
    subplot(2,2,i)
    plot(n,y), title(['Señal y[n] con SNR = ' num2str(SNR(s)) ' dB']), 
    xlabel('n')
    i=i+1;
end
subplot
...
while 1
    ...
    %Plocx(end+1)=sum(xl.^2); %Alternativamente
end
...
%SNRglobal=10*log10(sum(x.^2)/sum(r.^2)); %Alternativamente
SNRglobal=10*log10(sum(x.*x)/sum(r.*r)); %falta código
