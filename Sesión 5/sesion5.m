...
% A lo largo de este script de MATLAB hay bastante código faltante.
% Su tarea es completarlo. En donde falta código se añade el comentario:
               %%%% falta código
...
for N=Nvec
    ...
    k=(0:N-1); wk=2*pi*k/N;
    % Gráficas
    subplot(2,1,1), plot(wk/pi,20*log(abs(W))), grid %%%código aquí
    title(['Muestreo de ventana rectangular (N=',num2str(N),')']);
...
for N=Nvec
    ...
    H=fft(h,N);
    % Eje de frecuencias discretas
    k=(0:N-1); wk=2*pi*k/N;%%%código aquí
    % Gráficas
    subplot(2,1,1), plot(wk,20*log10(abs(H))), grid
    title(['Muestreo de ventana de Hamming (N=',num2str(N),')']);
    ylabel('|H(2\pik/L)|_d_B')
...
fprintf('Amplitud del coseno = %6.4f dB\n',2/L*10^(amp/20)) %%%código aquí
...
% a las componentes. Por tanto, P= y Q=
