...
stem(n,hsso,'g')
hold on
stem(hfilter,'Color','r') % añada código
grid, xlabel('n')
legend('h_{sso}[n]', 'h_{filter}[n]')
hold off
...
Fs=8000; % Añada código
Ts=1/Fs;
Fc=1000; % Añada código
...
ylabel('\angleH(\omega) (x \pi rad)'), grid %añadir código
...
[Blin,Alin]=zp2tf(clin,plin,k);% Añadir código
...
% Respuesta impulsiva con flujog(. . .)
% Respuesta impulsiva con filter
% Gráficas
% Comparación numérica
...
h=y;
 figure('Name','Flujograma','NumberTitle','off');
    ...
    stem(h_filter,'Color','r');
        legend('hflujo_g (n)', 'hfilter (n)','Location','northeast');
        hold off;
...
% Chebyshef tipo 1
[b3,a3]=cheby1(6, 0.5, wc);
 
[H3,w]=freqz(b3,a3,512);

% Chebyshef tipo 2
[b4,a4]=cheby2(6, 60, wc);
 
[H4,w]=freqz(b4,a4,512);


% Elíptico
[b5,a5]=ellip(6, 0.5, 60, wc);
 
[H5,w]=freqz(b5,a5,512);
...
% Diseño de un filtro Chebychef tipo 1
...
% Diagramas polo-ceros para 64, 32, 24 y 16 bits de precisión y forma
% directa

% Normalización y cuantifiación de los coeficientes B y A con 32, 24 y 16 bits

% Diagramas polos-ceros

% Estructura de secciones de segundo orden en serie
% Rediseño del filtro
